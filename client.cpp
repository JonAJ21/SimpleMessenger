...
int main(void)
{
    ...
    channel.declareQueue(login.c_str());


    std::thread th1([&]() {
        while(true) {
            std::cout << "Enter you message: {time} {login} {msg}" << std::endl;
            std::string time;
            std::string fr_login;
            std::string msg;
            std::cin >> time >> fr_login;
            getline(std::cin, msg, '\n');
            std::string send = time + "000 " + login + " " + fr_login + " " + msg;
            channel.publish("", "server_time", send.c_str());
        }
    });
    ...
    channel.consume(login.c_str(), AMQP::noack).onReceived(
